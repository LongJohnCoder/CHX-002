...
UINT16 FindTolum(IN ASIA_NB_CONFIGURATION *NbCfg)
{
  ...
  if(NbCfg->PciePE4){EnumerateBridgeDevice(0, 4, 0, &TotalSize);}  //D4F0
  if(NbCfg->PciePE5){EnumerateBridgeDevice(0, 4, 1, &TotalSize);}  //D4F1
  if(NbCfg->PciePE6){EnumerateBridgeDevice(0, 5, 0, &TotalSize);}  //D5F0
  if(NbCfg->PciePE7){EnumerateBridgeDevice(0, 5, 1, &TotalSize);}  //D5F1
