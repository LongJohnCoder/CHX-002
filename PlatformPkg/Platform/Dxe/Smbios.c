...
// A ~ Z
// a ~ z
STATIC BOOLEAN IsValidSpdAscii(CHAR8 c)
{
  if(c == ' ' || c == '(' || c == ')' || c == '-' || c == '.'){
    return TRUE;
  }
...
// return AsciiStrSize(Target)
STATIC UINTN CopyValidSpdPn(CHAR8 *Target, CHAR8 *Spd128)
{
  UINTN  Index;
  UINTN  Count = 0;

  for(Index=0; Index<18; Index++){
    if(IsValidSpdAscii(Spd128[Index])){
      Target[Count++] = Spd128[Index];
    }
  }
  Target[Count] = 0;
...
//DEBUG((EFI_D_INFO, "%a()\n", __FUNCTION__));
  ...
  for(Index=0;Index<DimmInfo->DimmCount;Index++){
    ...
    if(MemorySize == 0){
      ...
      Str += CopyValidSpdPn(Str, DimmInfo->SpdInfo[Index].PartNo);
