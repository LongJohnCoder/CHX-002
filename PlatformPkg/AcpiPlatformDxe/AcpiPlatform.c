  ...
  )
{
  ...
  switch (Table->Signature) {
	...
	//LNA-2016110801-S
	if(gSetupData->PcieASPMBootArch)
	{
	    DEBUG((EFI_D_ERROR,"Enable ASPM support \n"));
	}
	else
	{
	    DEBUG((EFI_D_ERROR,"Disable ASPM support \n"));
   	    pzxFadt->IaPcBootArch = (pzxFadt->IaPcBootArch) | EFI_ACPI_3_0_PCIE_ASPM_CONTROLS;
	}
	//LNA-2016110801-E
  ...
  )
{
  ...
  for (Index = 0; Index < FvHandleCount; Index++) {
    ...
    while(1){
	  ...
	  if(SIGNATURE_32('S', 'S', 'D', 'T') == *(UINT32 *)Sign)
	  {
	  	//JRZ: is it necessary to install this SSDT structure?
		if(SkipSSDTInstallation((EFI_ACPI_DESCRIPTION_HEADER *)CurrentTable))
		{
