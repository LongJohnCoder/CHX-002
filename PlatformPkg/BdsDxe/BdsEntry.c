...
#define VARIABLE_BUFFER_SIZE     512
...
EFI_STATUS VariableReclaimTest(UINTN Counter)
	{
		EFI_STATUS Status;
		UINT8 Index;
		UINT32 i;
	...
	DEBUG((EFI_D_ERROR, "\r\nSPI Variable Reclaim Test !.\r\n"));
		...
		////
		pWriteBuf = (UINT8*)AllocatePool(VARIABLE_BUFFER_SIZE); // Allocate buffer for set variable.
		pReadBuf  = (UINT8*)AllocatePool(VARIABLE_BUFFER_SIZE); // Allocate buffer for get variable.
		pWriteBuf[0] = 'C';
		pWriteBuf[1] = 'H';
		pWriteBuf[2] = 'X';
		pWriteBuf[3] = '0';
		...
		do
		{
			BufferSize= VARIABLE_BUFFER_SIZE;
			Index = 0;
			nCounter++;
			DEBUG((EFI_D_ERROR, "\r\nCounter = %d.\r\n", nCounter));
			for( Index = 0; Index < 4 ; Index++)
			{
				// Save variable - "CHX00200".
				pWriteBuf[VARIABLE_BUFFER_SIZE-1] = AsciiBuf[Index];
				//DEBUG((EFI_D_ERROR, "WriteBuf1 addr = 0x%x.\n", (UINTN)pWriteBuf));
				DEBUG((EFI_D_ERROR, "Save   Variable CHX00200..."));
				Status = gRT->SetVariable (
					L"CHX00200",
				...
				if (EFI_ERROR(Status)) {
					DEBUG((EFI_D_ERROR, ", Save Variable ret %r.\n\n",Status));
					return Status;
				} else {
				...
				// Verify the variable.
				//DEBUG((EFI_D_ERROR, "ReadBuf1 addr = 0x%x.\n",(UINTN)pReadBuf));
				DEBUG((EFI_D_ERROR, "Get Variable, "));
				Status = gRT->GetVariable(
					L"CHX00200",
					&gPlatformSetupVariableGuid,
					NULL,
				...
				if (EFI_ERROR(Status)) {
					DEBUG((EFI_D_ERROR, "Get Variable ret %r.\r\n",Status));
					return Status;
				}
				
				// Compare.
				DEBUG((EFI_D_ERROR, "Verify..."));
				for (i = 0; i < VARIABLE_BUFFER_SIZE; i++)
				{
					if (pReadBuf[i] != pWriteBuf[i]) {
						DEBUG((EFI_D_ERROR, "Copy&Compare failed.\r\n"));
						return EFI_SUCCESS;
					}
				}
				if (i == VARIABLE_BUFFER_SIZE) {
					DEBUG((EFI_D_ERROR, ", success!\r\n"));
				}
				
				// Save variable - "CHX00201".
				//DEBUG((EFI_D_ERROR, "WriteBuf2 addr = 0x%x.\n",(UINTN)pWriteBuf));
				DEBUG((EFI_D_ERROR, "Save   Variable CHX00201..."));
				Status = gRT->SetVariable (
					L"CHX00201",
					&gPlatformSetupVariableGuid,
					EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS,
				...
				// Verify the variable.
				DEBUG((EFI_D_ERROR, "Get Variable, "));
				//DEBUG((EFI_D_ERROR, "ReadBuf2 addr = 0x%x.\n",(UINTN)pReadBuf));
				Status = gRT->GetVariable(
					L"CHX00201",
					&gPlatformSetupVariableGuid,
					NULL,
				...
				// Delete variable - "CHX00200".
				DEBUG((EFI_D_ERROR, "Delete Variable CHX00200..."));
				Status = gRT->SetVariable (
					L"CHX00200",
					&gPlatformSetupVariableGuid,
					EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS,
				...
				if (EFI_ERROR(Status)) {
					DEBUG((EFI_D_ERROR, ", Delete Variable ret %r.\r\n",Status));
					return Status;
				} else {
					DEBUG((EFI_D_ERROR, ", success!\r\n",Status));
				}
				
				// Delete variable - "CHX00201".
				DEBUG((EFI_D_ERROR, "Delete Variable CHX00201..."));
				Status = gRT->SetVariable (
					L"CHX00201",
					&gPlatformSetupVariableGuid,
					EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_RUNTIME_ACCESS,
				...
				if (EFI_ERROR(Status)) {
					DEBUG((EFI_D_ERROR, ", Delte Variable ret %r.\r\n", Status));
					return Status;
				} else {
					DEBUG((EFI_D_ERROR, ", success!\r\n"));
				}
