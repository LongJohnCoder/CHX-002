...
//HYL2018032301-start
MicroSecondDelay(15); //66M is 5000ns, 27M is 15000ns, master attached mode, ECO enable
if (MmioRead8(0xFEB30000 + SPI0_ESPI_MASTER_SLAVE_ATTACHED_MODE_STA_REG)&0x10){
	DEBUG((EFI_D_INFO, "Now the system is eSPI master attached mode!!!\n"));
	while((MmioRead8(0xFEB30000 + SPI0_SPIS_REG)&0x01)){
		if (MmioRead8(0xFEB30000 + SPI0_SPIS_REG)&0x01){
//			DEBUG((EFI_D_INFO, "SPI Cycle in progress!!!\n"));
		}
	}
	DEBUG((EFI_D_INFO, "SPI Cycle not in progress!!!\n"));
	MmioWrite8(0xFEB30000 + SPI0_BIOS_BASE_ADR_BBAR, 0x01);
	DEBUG((EFI_D_INFO, "Try to get ownership: %X\n", MmioRead8(0xFEB30000 + SPI0_BIOS_BASE_ADR_BBAR)));
	while(((MmioRead8(0xFEB30000 + SPI0_BIOS_BASE_ADR_BBAR)&0x01)==0x00)){
//  		DEBUG((EFI_D_INFO, "SPI MMIO Rx50 is %X\n", MmioRead8(0xFEB30000 + 0x50))); 	
	}
	DEBUG((EFI_D_INFO, "Now has gotten the ownership: %X\n", MmioRead8(0xFEB30000 + SPI0_BIOS_BASE_ADR_BBAR)));	
}
//HYL2018032301-end
...
//HYL2018032301-start
if (MmioRead8(SpiBar + SPI0_ESPI_MASTER_SLAVE_ATTACHED_MODE_STA_REG)&0x10){
	MmioWrite8(SpiBar + SPI0_BIOS_BASE_ADR_BBAR, 0x02);
	DEBUG((EFI_D_INFO, "Release ownership\n"));
	while(MmioRead8(SpiBar + SPI0_BIOS_BASE_ADR_BBAR)){
//  		DEBUG((EFI_D_INFO, "SPI MMIO Rx50 is %X\n", MmioRead8(0xFEB30000 + SPI0_BIOS_BASE_ADR_BBAR))); 	
	}
}
//HYL2018032301-end	
  ...
  do{ 
	...
	MicroSecondDelay(15); //HYL2018032301 //66M is 5000ns, 27M is 15000ns, master attached mode, ECO enable
  ...
  if (MmioRead8(SpiBar + SPI0_ESPI_MASTER_SLAVE_ATTACHED_MODE_STA_REG)&0x10){
	DEBUG((EFI_D_INFO, "Now the system is eSPI master attached mode!!!\n"));
	while((MmioRead8(SpiBar + SPI0_SPIS_REG)&0x01)){
		if (MmioRead8(SpiBar + SPI0_SPIS_REG)&0x01){
//			DEBUG((EFI_D_INFO, "SPI Cycle in progress!!!\n"));
		}
	}
	DEBUG((EFI_D_INFO, "SPI Cycle not in progress!!!\n"));
	MmioWrite8(SpiBar + SPI0_BIOS_BASE_ADR_BBAR, 0x01);
	DEBUG((EFI_D_INFO, "Try to get ownership: %X\n", MmioRead8(SpiBar + SPI0_BIOS_BASE_ADR_BBAR)));
	while(((MmioRead8(SpiBar + SPI0_BIOS_BASE_ADR_BBAR)&0x01)==0x00)){
//  		DEBUG((EFI_D_INFO, "SPI MMIO Rx50 is %X\n", MmioRead8(SpiBar + 0x50))); 	
	}
	DEBUG((EFI_D_INFO, "Now has gotten the ownership: %X\n", MmioRead8(SpiBar + SPI0_BIOS_BASE_ADR_BBAR)));	
}
  //HYL2018032301-end
...
//  DEBUG((EFI_D_INFO, "(%d)%d %d\n", __LINE__, SpiDataCount, DataByteCount));
  ...
  MmioWrite8(SpiBar + SPI0_SPIC_REG,0x20);
...
if (MmioRead8(0xFEB30000 + SPI0_ESPI_MASTER_SLAVE_ATTACHED_MODE_STA_REG)&0x10){
	MmioWrite8(0xFEB30000 + SPI0_BIOS_BASE_ADR_BBAR, 0x02);
	DEBUG((EFI_D_INFO, "Release ownership\n"));
	while(MmioRead8(0xFEB30000 + SPI0_BIOS_BASE_ADR_BBAR)){
//  		DEBUG((EFI_D_INFO, "SPI MMIO Rx50 is %X\n", MmioRead8(0xFEB30000 + SPI0_BIOS_BASE_ADR_BBAR))); 	
	}
}
//HYL2018032301-end
