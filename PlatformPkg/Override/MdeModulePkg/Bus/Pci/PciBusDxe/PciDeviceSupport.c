  ...
  )
{
  ...
  UINT8               Data8;
  BOOLEAN             HasEfiImage;
  //Mike_20170216 Disable PCIE Complection in PXP _S
  #ifdef CHX002_PXP
  UINT8 DeviceControl2Offset;
  UINT8 DeviceControl2;
  #endif
  //Mike_20170216 Disable PCIE Complection in PXP _E
  //
  ...
  if (!EFI_ERROR (Status)) {
	...
	//Mike_20170216 Disable PCIE Complection in PXP _S
	#ifdef CHX002_PXP
	DEBUG((EFI_D_ERROR,"Found PCIE Device\n"));
	if((PciIoDevice->BusNumber > 0) && ((PciIoDevice->Pci.Hdr.ClassCode[2] == PCI_CLASS_MASS_STORAGE) && 
		(PciIoDevice->Pci.Hdr.ClassCode[1] == 06 /*PCI_CLASS_MASS_STORAGE_AHCI*/))) {
	//Found PCIE-SSD
	DEBUG((EFI_D_ERROR,"Found External AHCI Controller\n"));
	//Device Control2 is PciExpressCap's 28
	DeviceControl2Offset=PciExpressCapRegOffset+0x28;
	DEBUG((EFI_D_ERROR,"DeviceControl2Offset=%x\n",DeviceControl2Offset));
    ...
    DeviceControl2 |= 0x10; //Set Bit 4=1¡ê?disable completion timeout
	DEBUG((EFI_D_ERROR,"DeviceControl2=%x\n",DeviceControl2));
