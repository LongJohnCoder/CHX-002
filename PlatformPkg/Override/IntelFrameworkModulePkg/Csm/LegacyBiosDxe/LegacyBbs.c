...
// patch multi-AHCI port number issue.
  ...
  for (Index = 17; Index < 256; Index++) {
    ...
    PortBase = GetSataPortIndexBase(
                 BbsTable[Index].Bus, 
                 BbsTable[Index].Device, 
                 BbsTable[Index].Function
                 );
    if(PortBase == 0xFFFF){
      continue;
    }  
    ...
    AhciStr = AllocatePool(AsciiStrSize(SataNameDesc)+16);
    ...
    *SataNameDesc = 0;
    ...
    DEBUG((EFI_D_INFO, "ThisPortNo:%d, PortBase:%d\n", ThisPortIndex, PortBase));
    SataNameDesc++;
