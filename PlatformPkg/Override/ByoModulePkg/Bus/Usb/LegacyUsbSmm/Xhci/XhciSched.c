  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    XHCI_DEBUG ((EFI_D_ERROR, "XhcCreateUrb: XhcCreateTransferTrb Failed, Status = %r\n", Status));
    FreePool (Urb);
    Urb = NULL;
  }
  ...
  )
{
  ...
  //
  XhcWriteOpReg (Xhc, XHC_DCBAAP_OFFSET, XHC_LOW_32BIT(Dcbaa));
  XhcWriteOpReg (Xhc, XHC_DCBAAP_OFFSET + 4, XHC_HIGH_32BIT (Dcbaa));
  ...
  )
{
  ...
  if (EFI_ERROR(Status)) {
    XHCI_DEBUG ((EFI_D_ERROR, "XhcRecoverHaltedEndpoint: Reset Endpoint Failed, Status = %r\n", Status));
    goto Done;
  }
  ...
  if (EFI_ERROR(Status)) {
    XHCI_DEBUG ((EFI_D_ERROR, "XhcRecoverHaltedEndpoint: Set Transfer Ring Dequeue Pointer Failed, Status = %r\n", Status));
    goto Done;
  }
  ...
  )
{
 ...
 BOOLEAN Patch_flag=FALSE;//[ALJ20170503]for usb massstorage suprise remove during enumeration 
/**
  ...
  )
{
  ...
  //
  //[ALJ20170504]
  //when special case happened,disconnect will cause disable slot cmd happen,so set patch flag here.
  //
  if(Patch_flag==TRUE&&CheckedUrb->TrbStart->Type==TRB_TYPE_DIS_SLOT)
	{
	 XHCI_DEBUG ((EFI_D_ERROR, "Special case over\n"));
	 Patch_flag=FALSE;
   }
  //
  ...
  for (Index = 0; Index < Xhc->EventRing.TrbNumber; Index++) {
	...
	//
    //[ALJ20170503]
	//During USB masstorage enumeration,if remove device when masstorage driver is going,it will cause long delay for system recovery,and
	//system will not response for a long time;Here we add a flag to record this circumstance and process this issue quickly.
	//	
	if(EvtTrb->Type == TRB_TYPE_PORT_STATUS_CHANGE_EVENT&&
		(CheckedUrb->Ep.Type==XHC_CTRL_TRANSFER ||CheckedUrb->Ep.Type==XHC_BULK_TRANSFER||CheckedUrb->Ep.Type==0)
		&&(CheckedUrb->TrbStart->Type!=TRB_TYPE_EN_SLOT&&CheckedUrb->TrbStart->Type!=TRB_TYPE_DIS_SLOT))
	   {
		XHCI_DEBUG ((EFI_D_ERROR, "Special case happened\n"));
		Patch_flag=TRUE;
		break;
	 }
    //
    ...
    switch (EvtTrb->Completecode) {
      ...
      case TRB_COMPLETION_USB_TRANSACTION_ERROR:
       // CheckedUrb->Result  |= EFI_USB_ERR_TIMEOUT; //ttpandbg
        CheckedUrb->Result  |= EFI_USB_ERR_STALL;
        CheckedUrb->Finished = TRUE;
      ...
      case TRB_COMPLETION_SUCCESS:
        if (EvtTrb->Completecode == TRB_COMPLETION_SHORT_PACKET) {
          XHCI_DEBUG ((EFI_D_VERBOSE, "XhcCheckUrbResult: short packet happens!\n"));
        }
  ...
  )
{
  ...
  for (Index = 0; Index < Loop; Index++) {
    ...
    if (Finished||Patch_flag) {//if special case happened,just break to reduce  recovery time
      break;
    }
  ...
  if (Index == Loop) {
    ...
    Status      = EFI_DEVICE_ERROR;
  }else if(Patch_flag){// return ERROR for special case
	Urb->Result = EFI_USB_ERR_SYSTEM;
	Status		= EFI_DEVICE_ERROR;
  	}
  ...
  )
{
  ...
  if (((PortState->PortStatus & USB_PORT_STAT_ENABLE) != 0) &&
    ...
    } else if ((PortState->PortStatus & USB_PORT_STAT_SUPER_SPEED_PLUS) != 0) {
      Speed = EFI_USB_SPEED_SUPER_PLUS;
    }
    //
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    XHCI_DEBUG ((EFI_D_ERROR, "XhcInitializeDeviceSlot: Enable Slot Failed, Status = %r\n", Status));
    return Status;
  }
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    XHCI_DEBUG ((EFI_D_ERROR, "XhcInitializeDeviceSlot64: Enable Slot Failed, Status = %r\n", Status));
    return Status;
  }
  ASSERT (EvtTrb->SlotId <= Xhc->MaxSlotsEn);
  XHCI_DEBUG ((EFI_D_INFO, "Enable Slot Successfully, The Slot ID = 0x%x\n", EvtTrb->SlotId));
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    XHCI_DEBUG ((EFI_D_ERROR, "XhcDisableSlotCmd: Disable Slot Command Failed, Status = %r\n", Status));
    return Status;
  }
  //
  ...
  for (EpIndex = 0; EpIndex < NumEp; EpIndex++) {
    ...
    switch (EpDesc->Attributes & USB_ENDPOINT_TYPE_MASK) {
      ...
      case USB_ENDPOINT_ISO:
        ...
        //
        XHCI_DEBUG ((EFI_D_INFO, "XhcInitializeEndpointContext: Unsupport ISO EP found, Transfer ring is not allocated.\n"));
        EpDesc = (USB_ENDPOINT_DESCRIPTOR *)((UINTN)EpDesc + EpDesc->Length);
        continue;
      ...
      case USB_ENDPOINT_CONTROL:
        ...
        //
        XHCI_DEBUG ((EFI_D_INFO, "XhcInitializeEndpointContext: Unsupport Control EP found, Transfer ring is not allocated.\n"));
      default:
        XHCI_DEBUG ((EFI_D_INFO, "XhcInitializeEndpointContext: Unknown EP found, Transfer ring is not allocated.\n"));
        EpDesc = (USB_ENDPOINT_DESCRIPTOR *)((UINTN)EpDesc + EpDesc->Length);
        continue;
    }

    PhyAddr = (UINTN)((TRANSFER_RING *)(UINTN)Xhc->UsbDevContext[SlotId].EndpointTransferRing[Dci-1])->RingSeg0;
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    XHCI_DEBUG ((EFI_D_ERROR, "XhcSetConfigCmd: Config Endpoint Failed, Status = %r\n", Status));
  } else {
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    XHCI_DEBUG ((EFI_D_ERROR, "XhcSetConfigCmd64: Config Endpoint Failed, Status = %r\n", Status));
  } else {
    Xhc->UsbDevContext[SlotId].ActiveConfiguration = ConfigDesc->ConfigurationValue;
  }
  ...
  )
{
  ...
  XHCI_DEBUG ((EFI_D_INFO, "XhcStopEndpoint: Slot = 0x%x, Dci = 0x%x\n", SlotId, Dci));
  ...
  if (EFI_ERROR(Status)) {
    XHCI_DEBUG ((EFI_D_ERROR, "XhcStopEndpoint: Stop Endpoint Failed, Status = %r\n", Status));
  }
  ...
  )
{
  ...
  XHCI_DEBUG ((EFI_D_INFO, "XhcResetEndpoint: Slot = 0x%x, Dci = 0x%x\n", SlotId, Dci));
  ...
  if (EFI_ERROR(Status)) {
    XHCI_DEBUG ((EFI_D_ERROR, "XhcResetEndpoint: Reset Endpoint Failed, Status = %r\n", Status));
  }
  ...
  )
{
  ...
  XHCI_DEBUG ((EFI_D_INFO, "XhcSetTrDequeuePointer: Slot = 0x%x, Dci = 0x%x, Urb = 0x%x\n", SlotId, Dci, Urb));
  ...
  if (EFI_ERROR(Status)) {
    XHCI_DEBUG ((EFI_D_ERROR, "XhcSetTrDequeuePointer: Set TR Dequeue Pointer Failed, Status = %r\n", Status));
  }
  ...
  if ((IfDescActive != NULL) && (IfDescSet != NULL)) {
    ...
    for (EpIndex = 0; EpIndex < NumEp; EpIndex++) {
      ...
      if (Xhc->UsbDevContext[SlotId].EndpointTransferRing[Dci - 1] != NULL) {
        ...
        if (RingSeg != NULL) {
          UsbFreePool (RingSeg, sizeof (TRB_TEMPLATE) * TR_RING_TRB_NUMBER);
        }
        UsbFreePool (Xhc->UsbDevContext[SlotId].EndpointTransferRing[Dci - 1], sizeof (TRANSFER_RING));
    ...
    CmdTrbCfgEP.Type     = TRB_TYPE_CON_ENDPOINT;
    CmdTrbCfgEP.SlotId   = Xhc->UsbDevContext[SlotId].SlotId;
    XHCI_DEBUG ((EFI_D_INFO, "SetInterface: Configure Endpoint\n"));
    Status = XhcCmdTransfer (
               Xhc,
               (TRB_TEMPLATE *) (UINTN) &CmdTrbCfgEP,
               XHC_GENERIC_TIMEOUT,
    ...
    if (EFI_ERROR (Status)) {
      XHCI_DEBUG ((EFI_D_ERROR, "SetInterface: Config Endpoint Failed, Status = %r\n", Status));
    } else {
      //
  ...
  )
{
  ...
  if ((IfDescActive != NULL) && (IfDescSet != NULL)) {
    ...
    CmdTrbCfgEP.Type     = TRB_TYPE_CON_ENDPOINT;
    CmdTrbCfgEP.SlotId   = Xhc->UsbDevContext[SlotId].SlotId;
    XHCI_DEBUG ((EFI_D_INFO, "SetInterface64: Configure Endpoint\n"));
    Status = XhcCmdTransfer (
               Xhc,
               (TRB_TEMPLATE *) (UINTN) &CmdTrbCfgEP,
               XHC_GENERIC_TIMEOUT,
    ...
    if (EFI_ERROR (Status)) {
      XHCI_DEBUG ((EFI_D_ERROR, "SetInterface64: Config Endpoint Failed, Status = %r\n", Status));
    } else {
      //
      // Update the active AlternateSetting.
      //
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    XHCI_DEBUG ((EFI_D_ERROR, "XhcEvaluateContext: Evaluate Context Failed, Status = %r\n", Status));
  }
  return Status;
}
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    XHCI_DEBUG ((EFI_D_ERROR, "XhcEvaluateContext64: Evaluate Context Failed, Status = %r\n", Status));
  }
  return Status;
}
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    XHCI_DEBUG ((EFI_D_ERROR, "XhcConfigHubContext: Config Endpoint Failed, Status = %r\n", Status));
  }
  return Status;
}
