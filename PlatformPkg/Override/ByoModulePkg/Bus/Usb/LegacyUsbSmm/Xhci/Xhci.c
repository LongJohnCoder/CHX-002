  ...
  )
{
  ...
  case 5:
    PortStatus->PortStatus |= USB_PORT_STAT_SUPER_SPEED_PLUS;
    break;
  default:
    break;
  }
  ...
  *DataLength     = Urb->Completed;
  ...
  if (Status == EFI_TIMEOUT) {
    ...
    if (EFI_ERROR(RecoveryStatus)) {
      XHCI_DEBUG((EFI_D_ERROR, "XhcControlTransfer: XhcDequeueTrbFromEndpoint failed\n"));
    }
    goto FREE_URB;
  } else {
  ...
  if ((Request->Request     == USB_REQ_GET_DESCRIPTOR) &&
    ...
    PortStatus.PortStatus       = 0;
    PortStatus.PortChangeStatus = 0;
    if (DeviceSpeed == EFI_USB_SPEED_SUPER_PLUS) {
      //
    ...
    } else if (DeviceSpeed == EFI_USB_SPEED_FULL){
      if (XHC_BIT_IS_SET (State, BIT9)) {
        PortStatus.PortStatus |= USB_PORT_STAT_LOW_SPEED;
      } else {
    ...
    for (Index = 0; Index < MapSize; Index++) {
      if (XHC_BIT_IS_SET (State, mUsbHubClearPortChangeMap[Index].HwState)) {
        ...
        ClearPortRequest = UsbAllocatePool (sizeof (EFI_USB_DEVICE_REQUEST));
        ...
        UsbFreePool (ClearPortRequest, sizeof (EFI_USB_DEVICE_REQUEST));
      }
    }
  ...
  )
{
  ...
  *DataLength     = Urb->Completed;
  ...
  if (Status == EFI_TIMEOUT) {
    ...
    RecoveryStatus = XhcDequeueTrbFromEndpoint(Xhc, Urb);
    if (EFI_ERROR(RecoveryStatus)) {
      XHCI_DEBUG((EFI_D_ERROR, "XhcBulkTransfer: XhcDequeueTrbFromEndpoint failed\n"));
    }
  } else {
    if (*TransferResult == EFI_USB_NOERROR) {
      Status = EFI_SUCCESS;
      ...
      RecoveryStatus = XhcRecoverHaltedEndpoint(Xhc, Urb);
      if (EFI_ERROR (RecoveryStatus)) {
        XHCI_DEBUG ((EFI_D_ERROR, "XhcBulkTransfer: XhcRecoverHaltedEndpoint failed\n"));
      }
      Status = EFI_DEVICE_ERROR;
    }
    #else
      XHCI_DEBUG ((EFI_D_ERROR, "XhcBulkTransfer: XhcRecoverHaltedEndpoint\n"));
      RecoveryStatus = XhcRecoverHaltedEndpoint(Xhc, Urb);
      if (EFI_ERROR (RecoveryStatus)) {
        XHCI_DEBUG ((EFI_D_ERROR, "XhcBulkTransfer: XhcRecoverHaltedEndpoint failed\n"));
      }
      XHCI_DEBUG ((EFI_D_ERROR, "XhcBulkTransfer: XhcRecoverHaltedEndpoint Status=%x\n", Status));
...
--*/
{
  ...
  Xhc = XHC_FROM_THIS (This);
  Bda15 = (UINT8*)(UINTN)(0x415);  // int8 & xhci native driver issue sw smi for xhci keyboard flag
  ...
  if (!IsNewTransfer) {
    ...
    if ((Status == EFI_SUCCESS) && (mPeriodicFlag == 0)) {
      count --;
      if (count == 0) {
        *Bda15 = 0;
        if (BootCfg->BootStart) {
          // DEBUG((EFI_D_ERROR, "Set 8254 timer to 55ms\n"));
  ...
  if (mPeriodicFlag == 0) {
    ...
    if (count == 1) {
      *Bda15 = 1;
      // DEBUG((EFI_D_ERROR, "set 8254 timer to 10 ms\n"));
      // 0x2E9C = 10ms
      IoWrite8 (TIMER_CONTROL_PORT, TIMER0_CONTROL_WORD);
      IoWrite8 (TIMER0_COUNT_PORT, 0x9c);
      IoWrite8 (TIMER0_COUNT_PORT, 0x2e);
    }
  }
  ...
  if (PollingInterval < 0x10) {
    PollingInterval = 0x10;
  }
...
)
{
  ...
  UINT32                   Buffer;
  Xhc =  XHC_FROM_THIS(This);
  //XhcSetOpRegBit (Xhc, XHC_USBSTS_OFFSET, XHC_USBSTS_EINT);    // Set BIT3 to clear it,this will cleat PCD ,so change another way
  XhcWriteOpReg (Xhc, XHC_USBSTS_OFFSET, XHC_USBSTS_EINT);//[ALJ20170511]just Set BIT3 to clear it,do not clear PCD
  XhcSetRuntimeRegBit (Xhc, XHC_IMAN_OFFSET, XHC_IMAN_IP);
  ...
  if(Xhc->UsbLegSupOffset != 0xFFFFFFFF){
    ...
    Buffer |= (USBLEGCTLSTS_IOC_ENABLE | USBLEGCTLSTS_OWNERSHIP_ENABLE);    // USB SMI Enable, and SMI on OS Ownership Change Enable
    XhcWriteExtCapReg (Xhc, Xhc->UsbLegSupOffset + 4, Buffer); // Enable SMI and clear all status
  }
  
  XhcSetOpRegBit (Xhc, XHC_USBCMD_OFFSET, XHC_USBCMD_INTE);    // Enable interrupt

  return EFI_SUCCESS;
}
//ALJ20170511,for legacy support,move here
EFI_STATUS
EFIAPI
XhcEnableLegacySupport2 (
...
)
{
    ...
    // if ((UsbSts & USBSTS_PORT_CHANGE) &&
     if ((This->BusEnumerationCallback != NULL) &&
    ...
    XhcSetRuntimeRegBit (Xhc, XHC_IMAN_OFFSET, XHC_IMAN_IP); // Set IP to clear it
    return EFI_SUCCESS;
}
...
--*/
{
  ...
  if (Xhc->UsbMemBase & 0x04) {
    PciRegAddress += 4;
    Status = SmmPciCfgRead (
                 gSmst,
                 SmmPciWidthUint32,
                 (SMM_PCI_IO_ADDRESS *) &PciRegAddress,
                 (UINT8*)&Xhc->UsbMemBase + 4
             );          
    if (EFI_ERROR(Status)) {
        goto ON_ERROR;
    }
  ...
  XHCI_DEBUG ((EFI_D_INFO, "XhcCreateUsb3Hc: Capability length 0x%x\n", Xhc->CapLength));
  XHCI_DEBUG ((EFI_D_INFO, "XhcCreateUsb3Hc: HcSParams1 0x%x\n", Xhc->HcSParams1));
  XHCI_DEBUG ((EFI_D_INFO, "XhcCreateUsb3Hc: HcSParams2 0x%x\n", Xhc->HcSParams2));
  XHCI_DEBUG ((EFI_D_INFO, "XhcCreateUsb3Hc: HcCParams 0x%x\n", Xhc->HcCParams));
  XHCI_DEBUG ((EFI_D_INFO, "XhcCreateUsb3Hc: DBOff 0x%x\n", Xhc->DBOff));
  XHCI_DEBUG ((EFI_D_INFO, "XhcCreateUsb3Hc: RTSOff 0x%x\n", Xhc->RTSOff));
  XHCI_DEBUG ((EFI_D_INFO, "XhcCreateUsb3Hc: UsbLegSupOffset 0x%x\n", Xhc->UsbLegSupOffset));
  XHCI_DEBUG ((EFI_D_INFO, "XhcCreateUsb3Hc: DebugCapSupOffset 0x%x\n", Xhc->DebugCapSupOffset));

  Xhc->DevicePath = (EFI_DEVICE_PATH_PROTOCOL*)GetUsbPciDevPath(
                                                 (UINT8)PciAddress->Bus, 
                                                 (UINT8)PciAddress->Device,
                                                 (UINT8)PciAddress->Function
                                                 );
