  ...
  )
{
  ...
  *Consumed = 0;
  ...
  while ((Offset < Len) && (Head->Type != Type)) {
    Offset += Head->Len;
    if (Len <= Offset) {
      USB_DEBUG (( EFI_D_ERROR, "UsbCreateDesc: met mal-format descriptor, Beyond boundary!\n"));
      return NULL;
    }
    Head    = (USB_DESC_HEAD*)(DescBuf + Offset);
    if (Head->Len == 0) {
      USB_DEBUG (( EFI_D_ERROR, "UsbCreateDesc: met mal-format descriptor, Head->Len = 0!\n"));
      return NULL;
    }
  }
  ...
  )
{
  ...
  while ((Len >= sizeof (EFI_USB_INTERFACE_DESCRIPTOR)) && (EnumIf < NumIf ) ) {
    Setting = UsbParseInterfaceDesc (DescBuf, Len, &Consumed);

    if (Setting == NULL) {
      USB_DEBUG (( EFI_D_ERROR, "UsbParseConfigDesc: warning: failed to get interface setting, stop parsing now.\n"));
      break;
