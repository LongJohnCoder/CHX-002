  ...
  )
{
  ...
  if (TimerPeriod == 0) {
    ...
    if (mTimerConfiguration.Bits.MsiInterruptCapablity != 0 && FeaturePcdGet (PcdHpetMsiEnable)) {
      ...
      //
      DEBUG((EFI_D_ERROR,"HPET : Enable MSI Interrupt \n"));
      ...
      //
      DEBUG((EFI_D_ERROR,"HPET : Enable IO-APIC Interrupt \n"));
  ...
  )
{
  ...
  //set SB APIC use FSB
  MmioWrite8(0xFEC00000, 0x03);
  MmioWrite32(0xFEC00000+ 0x10, 0x01);
  
  
  HpetBar = MmioRead32(LPC_PCI_REG(LPC_HPET_CTRL_REG));
  //enable hpet and set via mode
  MmioWrite32(LPC_PCI_REG(LPC_HPET_CTRL_REG),(MmioRead32(LPC_PCI_REG(LPC_HPET_CTRL_REG))|BIT7)&(~BIT4));
  //DEBUG ((DEBUG_ERROR, "D17F0 Rx68=0x%x\n",MmioRead32(LPC_PCI_REG(0x68))));
  HpetBar &= (~0xFFFFFC80);
  HpetBar |= 0xFED00000 |0x80;
  MmioWrite32(LPC_PCI_REG(LPC_HPET_CTRL_REG), HpetBar);
  HpetBar = 0;
  HpetBar = MmioRead32(LPC_PCI_REG(LPC_HPET_CTRL_REG));
 ///MTN-2016110201-End
  //
