...
EFI_PEI_PPI_DESCRIPTOR mPpiListAcpiWakeVectorTable = {
  (EFI_PEI_PPI_DESCRIPTOR_PPI | EFI_PEI_PPI_DESCRIPTOR_TERMINATE_LIST),
  ...
  )
{
  ...
  Status = PeiServicesInstallPpi (&mPpiListAcpiWakeVectorTable);
  ASSERT_EFI_ERROR (Status);
  ...
  )
{
  if (FeaturePcdGet (PcdDxeIplSwitchToLongMode)) {
    ...
    //
    DEBUG ((EFI_D_INFO, "S3NvsPageTableAddress - %x\n", S3NvsPageTableAddress));
  ...
  )
{
  ...
  REPORT_STATUS_CODE (
    EFI_PROGRESS_CODE, 
    (EFI_SOFTWARE_PEI_MODULE | EFI_SW_PEI_PC_S3_BOOT_SCRIPT)
    );
  ...
  )
{
  ...
  REPORT_STATUS_CODE (
    EFI_PROGRESS_CODE, 
    (EFI_SOFTWARE_PEI_MODULE | EFI_SW_PEI_PC_S3_STARTED)
    );
