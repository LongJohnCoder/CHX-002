  ...
  for (Index = 0; Index < PeiUsbDevice->DownStreamPortNo; Index++) {
    ...
    if (IsPortConnectChange (PortStatus.PortChangeStatus)) {
      ...
      if (IsPortConnect (PortStatus.PortStatus)) {
        ...
        ParentRouteChart = PeiUsbDevice->RouteChart;
         if (ParentPort < 14) {
          NewPeiUsbDevice->RouteChart.Route.RouteString = ParentRouteChart.Route.RouteString |
                                                           ((ParentPort) << (4 * (ParentRouteChart.Route.TierNum - 1)));
        } else {
           NewPeiUsbDevice->RouteChart.Route.RouteString = ParentRouteChart.Route.RouteString |
                                                           (15 << (4 * (ParentRouteChart.Route.TierNum - 1)));
        }
        NewPeiUsbDevice->RouteChart.Route.RootPortNum  = ParentRouteChart.Route.RootPortNum;
        NewPeiUsbDevice->RouteChart.Route.TierNum      = ParentRouteChart.Route.TierNum + 1;
       ...
       if (PeiUsbDevice->DeviceSpeed != EFI_USB_SPEED_SUPER ) {
        ...
        if (PeiUsbDevice->DeviceSpeed == EFI_USB_SPEED_SUPER) {
          ...
          NewPeiUsbDevice->DeviceSpeed = GetUsbDeviceSpeed (PortStatus.PortStatus);
         }
        ...
        //
        // Do Xhc-Specified action
        //

        if (PeiUsbDevice->HostController == PEI_XHCI_CONTROLLER) {
          NewPeiUsbDevice->HostController = PEI_XHCI_CONTROLLER;
          Status= NewPeiUsbDevice->Usb2HcPpi->Usb3HostControllerFunc.ApplyDeviceSlot (
                                                                       PeiServices,
                                                                       NewPeiUsbDevice->Usb2HcPpi,
                                                                       NewPeiUsbDevice->RouteChart,
                                                                       NewPeiUsbDevice->DeviceSpeed,
                                                                       PeiUsbDevice->DeviceAddress,
                                                                       &(NewPeiUsbDevice->DeviceAddress)
                                                                       );
          if (EFI_ERROR (Status)) {
            return Status;
          }
  ...
  for (Index = 0; Index < NumOfRootPort; Index++) {
    ...
    if (IsPortConnectChange (PortStatus.PortChangeStatus)) {
      ...
      if (IsPortConnect (PortStatus.PortStatus)) {
        ...
        PeiUsbDevice->RouteChart.Route.RouteString  = 0;
        PeiUsbDevice->RouteChart.Route.RootPortNum  = Index+1;
        PeiUsbDevice->RouteChart.Route.TierNum  = 1;
        PeiUsbDevice->DeviceSpeed= GetUsbDeviceSpeed(PortStatus.PortStatus);
        if (Usb2HcPpi != NULL) {
          Usb2HcPpi->GetHostControllerType (
                        PeiServices,
                        Usb2HcPpi,
                        &HcType
                        );
          PeiUsbDevice->HostController = HcType;
        }
        if (PeiUsbDevice->DeviceSpeed != EFI_USB_SPEED_SUPER ) {
          ResetRootPort (
            PeiServices,
        ...
        if (Usb2HcPpi != NULL) {
          ...
          UsbHcPpi->GetRootHubPortStatus (
                ...
                if (!(PortStatus.PortStatus &USB_PORT_STAT_ENABLE)) {
                    if ( PeiUsbDevice->HostController == PEI_EHCI_CONTROLLER ) {
                       ...
                       DEBUG ((EFI_D_INFO, "Release Low speed device to U/Ohci! \n"));
                    }
                    DEBUG ((EFI_D_INFO, "The port hasn't been enabled! \n"));
                    continue;
                }
                PeiUsbDevice->DeviceSpeed = GetUsbDeviceSpeed (PortStatus.PortStatus);
                DEBUG ((EFI_D_INFO, "Device Speed ReGet=%d\n", PeiUsbDevice->DeviceSpeed));
                //
                //Do Xhc-Specified action
                //
                if (PeiUsbDevice->HostController == PEI_XHCI_CONTROLLER) {
                    Status=Usb2HcPpi->Usb3HostControllerFunc.ApplyDeviceSlot (
                               PeiServices,
                               Usb2HcPpi,
                               PeiUsbDevice->RouteChart,
                               PeiUsbDevice->DeviceSpeed,
                               0,
                               &(PeiUsbDevice->DeviceAddress)
                           );
                    ...
                    DEBUG ((EFI_D_INFO, "xhc downstream PeiUsbDevice->DeviceAddress=%x!\n",PeiUsbDevice->DeviceAddress));
                }
  ...
  )
{
  ...
  if (PeiUsbDevice->HostController == PEI_XHCI_CONTROLLER) {
    Status = PeiUsbDevice->Usb2HcPpi->Usb3HostControllerFunc.EvaluateContext (
                                                               PeiServices,
                                                               PeiUsbDevice->Usb2HcPpi,
                                                               PeiUsbDevice->DeviceAddress,
                                                               FALSE,
                                                               ...
                                                               0,
                                                               0,
                                                               (UINT32)PeiUsbDevice->MaxPacketSize0,
                                                               0
                                                               );
    if (EFI_ERROR (Status)) {
      return Status;
  ...
  //
  // Do the final Xhc Configure Endpoint command to sync the selected Endpoints
  // 4.6.6 Configure Endpoint
  // The xHC does not maintain information about the relationships between the Configuration
  ...
  //
  if (PeiUsbDevice->HostController == PEI_XHCI_CONTROLLER) {
    Status =  PeiUsbDevice->Usb2HcPpi->Usb3HostControllerFunc.SetConfiguration (
                                                                PeiServices,
                                                                PeiUsbDevice->Usb2HcPpi,
                                                                PeiUsbDevice->DeviceAddress,
                                                                PeiUsbDevice->EndpointDesc,
                                                                PeiUsbDevice->InterfaceDesc->NumEndpoints
                                                                );
    if (EFI_ERROR (Status)) {
      return EFI_TIMEOUT;
    }
  ...
  )
{
  ...
  if (Usb2HcPpi != NULL) {
    ...
    MicroSecondDelay ((RetryIndex + 1) * 100 * 1000);
  } else {
    MicroSecondDelay (200 * 1000);
...
--*/
{
  ...
  } else if ((PortStatus & USB_PORT_STAT_FULL_SPEED) != 0) {
