  ...
  )
{
  ...
  EFI_BLOCK_IO_MEDIA          *Media;
  EFI_STATUS                  Status;
  static UINT8                USBDeviceNum = 0;
  UINT8                       Count;
  BOOLEAN                     FlagNoRetry = FALSE;
  BOOLEAN                     FlagNotASCII = FALSE;
  BOOLEAN                     FlagVendorSpace =FALSE;
  BOOLEAN                     FlagProductSpace =FALSE;

  Media = &(UsbMass->BlockIoMedia);
  for(Count = 0; Count < 8; Count++){
    if(UsbMass->InquiryData.VendorID[Count]) {
      break;
    }
  }
  if(Count == 8){
  ...
  for(Count = 0; Count < 8; Count++){
    if(UsbMass->InquiryData.VendorID[Count] & 0x80){
      FlagNotASCII = TRUE;
    }
   }
  for(Count = 0; Count < 16; Count++){
    if(UsbMass->InquiryData.ProductID[Count] & 0x80){
      FlagNotASCII = TRUE;
    }
   }
//fix bug for usb device whose IdProduct and IdVendor is full zero 	
//determine if vendorID is full space
  for (Count = 0; Count < 8; Count ++) {
    if (UsbMass->InquiryData.VendorID[Count] != ' ')
      break;
    }
  if (Count == 8) {
     FlagNoRetry = TRUE;
     FlagVendorSpace = TRUE;
  }
//determine if ProductID is full space 

  for(Count = 0; Count < 16; Count++){
    if(UsbMass->InquiryData.ProductID[Count] !=' ')
	break; 
    }
  if(Count==16){
      FlagProductSpace = TRUE;
  }
  
  if(FlagVendorSpace&&FlagProductSpace){
      FlagNotASCII=TRUE;
  }

  
  if(FlagNoRetry && FlagNotASCII) {
     USBDeviceNum++;
     if(USBDeviceNum > 9){
     USBDeviceNum = 9;
     }
     UsbMass->InquiryData.VendorID[0] = 'U';
     UsbMass->InquiryData.VendorID[1] = 'S';
     UsbMass->InquiryData.VendorID[2] = 'B';
     UsbMass->InquiryData.VendorID[3] = ' ';
     UsbMass->InquiryData.VendorID[4] = ' ';
     UsbMass->InquiryData.VendorID[5] = ' ';
     UsbMass->InquiryData.VendorID[6] = ' ';
     UsbMass->InquiryData.VendorID[7] = ' ';
     UsbMass->InquiryData.ProductID[0] = 'F';
     UsbMass->InquiryData.ProductID[1] = 'l';
     UsbMass->InquiryData.ProductID[2] = 'a';
     UsbMass->InquiryData.ProductID[3] = 's';
     UsbMass->InquiryData.ProductID[4] = 'h';
     UsbMass->InquiryData.ProductID[5] = ' ';
     UsbMass->InquiryData.ProductID[6] = 'D';
     UsbMass->InquiryData.ProductID[7] = 'i';	  
     UsbMass->InquiryData.ProductID[8] = 's';
     UsbMass->InquiryData.ProductID[9] = 'k';
     UsbMass->InquiryData.ProductID[10] = ' ';
     UsbMass->InquiryData.ProductID[11] = 0x30+USBDeviceNum;
     UsbMass->InquiryData.ProductID[12] = ' ';
     UsbMass->InquiryData.ProductID[13] = ' ';
     UsbMass->InquiryData.ProductID[14] = ' ';
     UsbMass->InquiryData.ProductID[15] = ' ';
    }
