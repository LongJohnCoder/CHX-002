...
{
  IoWrite16(Uhc->UsbHostControllerBaseAddress + Offset, Data);
}
...
{
   return IoRead16(Uhc->UsbHostControllerBaseAddress + Offset);
}
  ...
  )
{
  ...
  Data = IoRead16 (Uhc->UsbHostControllerBaseAddress + Offset);
  ...
  )
{
  ...
  UhciClearRegBit (Uhc, USBCMD, USBCMD_RS);
  ...
  )
{
  ...
  //
  Status = UhciStopHc (UhcDev, 50 * STALL_1_MILLI_SECOND);
  ASSERT_EFI_ERROR (Status);

  UhciSetRegBit (UhcDev, USBCMD, USBCMD_GRESET);
  MicroSecondDelay (10 * STALL_1_MILLI_SECOND);

  //
  // Clear the Global Reset bit to zero.
  //
  UhciClearRegBit (UhcDev, USBCMD, USBCMD_GRESET);
  ...
  //
  UhciWriteReg (UhcDev, USBCMD, USBCMD_RS | USBCMD_MAXP | USBCMD_CF);
