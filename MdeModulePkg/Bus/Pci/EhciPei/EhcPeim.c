  ...
  )
{
  ...
  MicroSecondDelay (EHC_ROOT_PORT_RECOVERY_STALL * 100); // stall 2s for ADATA3.0 device
  ...
  Status = EhcEnablePeriodSchd (Ehc, EHC_GENERIC_TIMEOUT * 10);
 ...
 )
{
  ...
  //
  PciStatus = PciRead16 (PCI_LIB_ADDRESS(Bus, Device, Function, PCI_PRIMARY_STATUS_OFFSET));
  ...
  //
  CapabilityPtr = PciRead8 (PCI_LIB_ADDRESS(Bus, Device, Function, PCI_CAPBILITY_POINTER_OFFSET));
  //
  // Find Capability ID 0xA, Which Is For Debug Port
  //
  while (CapabilityPtr != 0) {
    CapabilityId = PciRead8 (PCI_LIB_ADDRESS(Bus, Device, Function, CapabilityPtr));
    if (CapabilityId == 0x0A) {
      break;
    }
    CapabilityPtr = PciRead8 (PCI_LIB_ADDRESS(Bus, Device, Function, CapabilityPtr + 1));
  }
  ...
  //
  DebugPort = PciRead16 (PCI_LIB_ADDRESS(Bus, Device, Function, CapabilityPtr + 2));
  ...
  )
{
  ...
  MemoryBar = PciRead32 (PCI_LIB_ADDRESS(Bus, Device, Function, EHC_PCI_MEM_BASE_ADDRESS + 4 * Ehc->DebugPortBarNum));

  return MmioRead32 (MemoryBar + Ehc->DebugPortOffset);
}
  ...
  )
{
  ...
  while (TRUE) {
    ...
    EhcDev->Usb2HostControllerPpi.GetHostControllerType    = EhcGetHostControllerType;
  ...
  )
{
  ...
  EhcGetUsbDebugPortInfo (EhcDev, Bus, Device, Function);
  if (EhcDev->DebugPortNum != 0) {
    State = EhcReadDbgRegister (EhcDev, Bus, Device, Function);
