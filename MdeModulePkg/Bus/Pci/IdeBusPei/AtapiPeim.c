  ...
  )
{
  ...
  if (NeedReadCapacity) {
    ...
    for (Index = 0; (Index < RetryNum) && (Index < MaxRetryNum); Index++) {
      ...
      if (Status != EFI_SUCCESS) {
        DEBUG((DEBUG_ERROR, "ReadCapacity Index:%d\n", Index));
        MicroSecondDelay (1000000);  // stall 1s for CDROM spin-up
  ...
  )
{
  ...
  *SenseCounts = 0;
  ...
  for (SenseReq = TRUE; SenseReq;) {
    ...
    //
    if ((Sense->sense_key != ATA_SK_NO_SENSE) && ((*SenseCounts) < MAX_SENSE_KEY_COUNT)) {
