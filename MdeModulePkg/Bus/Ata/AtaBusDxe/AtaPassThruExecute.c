...
// Some CF cards wrong report config.BIT15 as 1 (0 = ATA device).
// So we should ignore it.
// Typically, SanDisk 256MB CompactFlash config is 0x848A
//
// Some ODDs' Indentify.Config are as below:
//   sony dvd rw ad-7240s		    0x85C0
//   asus dvd-e818a7t		        0x85C0
//   HL-DT-ST DVDRAM GH40N		  0x85C0
//   PIONEER DVD-ROM DVD-227P	  0x85C0
//   HL-DT-ST DVDRAM GH22NS50	  0x85C0
// So this magic word may be safe.

    if (IdentifyData->config == 0x848A) {
      DEBUG ((DEBUG_ERROR, "Meet CF card with bad Identify.Config.BIT15\n"));
    } else {
      //
  ...
  if((IdentifyData->command_set_supported_82 & BIT1) &&      // support
    ...
    AtaDevice->Locked = TRUE;                                // default FALSE.
    DEBUG((EFI_D_INFO, "ATA Locked\n"));		
  }
