...
#include "PeiMain.h"
//MIKE_Dump Default_Value  
//1: Dump
//0: Not Dump
#define DUMP_DEAULT_VAULE    0

EFI_PEI_PPI_DESCRIPTOR mPeiDumpRegPpi = {
  (EFI_PEI_PPI_DESCRIPTOR_PPI | EFI_PEI_PPI_DESCRIPTOR_TERMINATE_LIST),
  &gEfiPeiDumpRegPpiGuid,
  (VOID *) &mPeiDumpReg
};
...
EFI_PEI_PPI_DESCRIPTOR mFirmwareLoadPpi = {
  (EFI_PEI_PPI_DESCRIPTOR_PPI | EFI_PEI_PPI_DESCRIPTOR_TERMINATE_LIST),
  ...
  )
{
  ...
  if (OldCoreData == NULL) {
    //
    // Report Status Code EFI_SW_PEI_CORE_PC_ENTRY_POINT
    //
    REPORT_STATUS_CODE (
      EFI_PROGRESS_CODE,
      (EFI_SOFTWARE_PEI_CORE | EFI_SW_PEI_CORE_PC_ENTRY_POINT)
      );
	...
	// 20170118 HXZ debug. for reg dump
	if(1==DUMP_DEAULT_VAULE){
	 Status = PeiServicesInstallPpi (&mPeiDumpRegPpi);
     ASSERT_EFI_ERROR(Status);
	}
	  //
  // PEI dump Reg PPI
  ...
  if(1==DUMP_DEAULT_VAULE){
    ...
    //JIH-20161202 FW Notify, Copy Form IOE CJW code
    Status = PeiServicesInstallPpi(&mFirmwareLoadPpi);
