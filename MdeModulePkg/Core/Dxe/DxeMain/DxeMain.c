...
//LGE-20170815 DXE MEMTEST CODE -S
UINT32 rotate_left(UINT32 pat, UINT32 shift)
{
	UINT32 i = 0, tmp = 0;
...
UINT8 ZXVisaMemoryRead8(UINT64 Address)
{ return *((volatile UINT8*)(UINTN)Address); }
UINT16 ZXVisaMemoryRead16(UINT64 Address)
{ return *((volatile UINT16*)(UINTN)Address); }
UINT32 ZXVisaMemoryRead32(UINT64 Address)
{ return *((volatile UINT32*)(UINTN)Address); }
UINT64 ZXVisaMemoryRead64(UINT64 Address)
{ return *((volatile UINT64*)(UINTN)Address); }

VOID ZXVisaMemoryWrite8(UINT64 Address, UINT8 Data)
{ *((volatile UINT8*)(UINTN)Address) = Data; }
VOID ZXVisaMemoryWrite16(UINT64 Address, UINT16 Data)
{ *((volatile UINT16*)(UINTN)Address) = Data; }
VOID ZXVisaMemoryWrite32(UINT64 Address, UINT32 Data)
{ *((volatile UINT32*)(UINTN)Address) = Data; }
VOID ZXVisaMemoryWrite64(UINT64 Address, UINT64 Data)
                     ...
                     IN UINT32		Pattern)
{
	...
	UINT32 Rdata,Rdata1;
	BOOLEAN bret = FALSE;
    UINT32 BaseAddrMB, EndAddrMB;

	BaseAddrMB = (UINT32)(StartAddr/0x100000);
	EndAddrMB = (UINT32)((StartAddr + TestLen)/0x100000);

    testlen = 0x100000;  // 1MB
    
   DEBUG(( EFI_D_ERROR,"\nBaseAddress = 0x%8x MB EndAddress = 0x%8x MB,    Length = 0x%8x   ,Pattern = 0x%8x \n",
		BaseAddrMB, EndAddrMB,  testlen, Pattern));	
	//write pattern
	for(i = BaseAddrMB; i< EndAddrMB; i++){
         Buf = MultU64x32(i,0x100000);
		 for(j = Buf; j < Buf + testlen; j+=4)
		   ZXVisaMemoryWrite32(j, Pattern);
	}
	...
	for(i = BaseAddrMB; i< EndAddrMB; i++){
			 Buf = MultU64x32(i,0x100000);
			 for(j = Buf; j < Buf + testlen; j+=4){
                Rdata = ZXVisaMemoryRead32(j);
				if (Rdata != Pattern)
               {
                //IoWrite8(0x80,0x13);
				ZXVisaMemoryWrite32(j, Pattern);
				Rdata1 = ZXVisaMemoryRead32(j);				
               DEBUG(( EFI_D_ERROR, "\n1.ERROR Address=0x%08x,Error Data =0x%08x\n", (UINT32)j, Rdata));
		        bret = TRUE;		        
		        goto Error;	
    ...
    for(i = BaseAddrMB; i< EndAddrMB; i++){
			 ...
			 for(j = Buf; j < Buf + testlen; j+=4){
				...
				if (Rdata != ~Pattern)
               {
				...
				ZXVisaMemoryWrite32(j, ~Pattern);
				Rdata1 = ZXVisaMemoryRead32(j);	
                DEBUG((EFI_D_ERROR, "\n2.ERROR Address=0x%08x,Error Data =0x%08x\n", (UINT32)j, Rdata));
		        bret = TRUE;		        
		        goto Error;	
...
BOOLEAN DXEAddrtest(
                     IN UINT64                        Start, // unit of 1
                     IN UINT64                        Length  // unit of 1
                    )
{
    UINT64 i;
   ...
   DEBUG(( EFI_D_ERROR,"Address Test:Start = 0x%08x%08x  Length = 0x%016x   \n",(UINT32)(Start>>32),(UINT32)(Start&0Xffffffff), (UINT32)Length));
	//write pattern
	for(i = 0; i< Length; i+=4){
        
		   ZXVisaMemoryWrite32(Start + i, (UINT32)(Start + i));
	}


    //read pattern 
	for(i = 0; i< Length; i+=4){			
                Rdata = ZXVisaMemoryRead32(Start + i);
				if (Rdata != (UINT32)(Start + i))
               {
                //IoWrite8(0x80,0x13);
				//ZXVisaMemoryWrite32(Start + i, (UINT32)(Start + i));
				//Rdata1 = ZXVisaMemoryRead32(Start + i);				
               DEBUG(( EFI_D_ERROR, "\n1.ERROR Address=0x%08x,Error Data =0x%08x\n", (UINT32)(Start + i), (UINT32)Rdata));
		        bret = TRUE;       
...
VOID RandMemTest(UINT32 StartAddr,UINT32  TestLength)
{
     UINT32 i; 
	 ...
	 UINT32 ReadData;
	 UINT32 EndAddr;
     DEBUG((EFI_D_ERROR,"Random Memory Test"));
//Init SEED
     SEED_X = 521288629; 
	 SEED_Y = 362436069;
     ...
     for(i = 0;i < TestLength; i+=4)
     	{
		   ...
		   //Ensure   TestAddr%4 = 0
		   TestAddr &= 0xFFFFFFF4;
		   ZXVisaMemoryWrite32(TestAddr, TestAddr);
     ...
     for(i = 0;i < TestLength; i+=4)
     	{
		   ...
		   //Ensure   TestAddr%4 = 0
		   TestAddr &= 0xFFFFFFF4;
		   ReadData = ZXVisaMemoryRead32(TestAddr);

           if(ReadData != TestAddr){
		   	 DEBUG(( EFI_D_ERROR,"\nerror! address 0x%08x = 0x%08x, should be 0x%08x\r\n",
...
VOID JumpMemTest(UINT64 StartAddr, UINT32 TestLen, UINT32 JumpStep)
{
	...
	DEBUG(( EFI_D_ERROR,"\nBegin Jump Memory Test: StartAddr=0x%08x, TestLen=0x%08x, JumpStep=0x%08x",(UINT32)StartAddr, TestLen, JumpStep));
    ...
    for(i = 0;i < JumpStep; i += 4){
	   ...
	   for(j = i; j < TestLen; j+= JumpStep)
		{
		   ZXVisaMemoryWrite32(StartAddr + j, Pattern);
	...
	for(i = 0;i < JumpStep; i += 4){
	   ...
	   for(j = i; j < TestLen; j+= JumpStep)
		{
		   ReadData = ZXVisaMemoryRead32(StartAddr + j);
...
VOID JumpAddrTest(UINT64 StartAddr, UINT32 TestLen, UINT32 JumpStep)
{

      UINT32 i,j;
	  UINT32 ReadData,ReadData1;
	UINT8 readerror;
	DEBUG(( EFI_D_ERROR,"\nBegin Jump Address Test: StartAddr=0x%08x, TestLen=0x%08x, JumpStep=0x%08x",(UINT32)StartAddr, TestLen, JumpStep));
    ...
    //for(i = 0;i < 1; i += 4){
	   for(j = i; j < TestLen; j+= JumpStep)
		{
		   ZXVisaMemoryWrite32(StartAddr + j, (UINT32)(StartAddr + j));
	...
	//for(i = 0;i < 1; i += 4){
	   ...
	   for(j = i; j < TestLen; j+= JumpStep)
		{
		   ReadData = ZXVisaMemoryRead32(StartAddr + j);
           ReadData1 = ZXVisaMemoryRead32(StartAddr + j);
		   if(ReadData != ReadData1)
		   	{
		   	   //IoWrite8(0x80, 0x13);

			   //trigger write 0x100000
			   ZXVisaMemoryWrite32(0x100000, 0x5A5A5A5A);   
		   	   readerror = 1;
			   DEBUG(( EFI_D_ERROR,"\nread error! address 0x%08x = 0x%08x, should be 0x%08x\r\n",
				(UINT32)(StartAddr + j),(UINT32)ReadData,(UINT32)(StartAddr + j)));
		   	}
		   else 
		   	 readerror = 0;
		   
		   if((readerror == 0) && (ReadData != (UINT32)(StartAddr + j))){
		   	     //IoWrite8(0x80, 0x13);

				  //trigger write 0x110000
			   ZXVisaMemoryWrite32(0x110000, 0x5A5A5A5A); 
		   	    DEBUG(( EFI_D_ERROR,"\nwrite error! address 0x%08x = 0x%08x, should be 0x%08x\r\n",
				(UINT32)(StartAddr + j),(UINT32)ReadData,(UINT32)(StartAddr + j)));}   
...
VOID JumpAddrTest1(UINT64 StartAddr, UINT32 TestLen, UINT32 JumpStep)
{

      UINT32 i,j;
	  UINT32 ReadData,ReadData1;
	UINT8 readerror;
	DEBUG(( EFI_D_ERROR,"\nBegin Jump Address Test NEW: StartAddr=0x%08x, TestLen=0x%08x, JumpStep=0x%08x",(UINT32)StartAddr, TestLen, JumpStep));
...
VOID Write2ReadTest( UINT64 StartAddr, UINT32 TestLen, UINT32 JumpStep)
{
   ...
   DEBUG(( EFI_D_ERROR,"\nBegin Write to Read Test: StartAddr = 0x%08x, TestLen = 0x%08x\r\n",StartAddr,TestLen));


   DEBUG(( EFI_D_ERROR,"\nSimple Write to Read test\r\n")); 
   for(i = 0;i < TestLen; i += 4)
   {
  
      ZXVisaMemoryWrite32(StartAddr + i, Pattern);
      ReadData = ZXVisaMemoryRead32(StartAddr + i);
	  if(ReadData != Pattern)
	  	{
	  	//IoWrite8(0x80,0x13);
	  	 DEBUG(( EFI_D_ERROR,"\nerror! address 0x%08x = 0x%08x, should be 0x%08x\r\n",
   ...
   for(i = JumpStep; i < TestLen; i += JumpStep)
   {
	  ...
	  for(j = i; j < i+JumpStep; j+= 4)
	      ...
	      //Read   i-JumpStep ---> i
		  ReadData = ZXVisaMemoryRead32( StartAddr + j - JumpStep);
		  if(ReadData != Pattern){
               //IoWrite8(0x80,0x13);
               DEBUG(( EFI_D_ERROR,"\nerror! address 0x%08x = 0x%08x, should be 0x%08x\r\n",
...
VOID DXEBurnIn()
{
  ...
  #if 0
   DEBUG(( EFI_D_ERROR, "Begin 4GB~6GB Cross bank Test\r\n"));
    StartAddr = 0x100000000;
	TestLen = 0x2000000;
	JumpStep = 0x8000;
	//JumpMemTest(StartAddr, TestLen, JumpStep);
    
	DEBUG(( EFI_D_ERROR, "Begin 4GB~6GB Cross Page Test\r\n"));    
	StartAddr = 0x100000000;
	//TestLen = 0x80000000;
	JumpStep = 0x20000;
	//JumpMemTest(StartAddr, TestLen, JumpStep);
 
    DEBUG(( EFI_D_ERROR, "Begin 4GB~6GB Cross BankGroup Test\r\n"));
    StartAddr = 0x100000000;
	//TestLen = 0x80000000;
	JumpStep = 0x40;
	//JumpMemTest(StartAddr, TestLen, JumpStep);   
    ...
    DEBUG(( EFI_D_ERROR, "Begin 4GB~6GB Cross bank Address Test\r\n"));
    StartAddr = 0x100000000;
	//TestLen = 0x80000000;
	JumpStep = 0x8000;
	//JumpAddrTest(StartAddr, TestLen, JumpStep);
    JumpAddrTest1(StartAddr, TestLen, JumpStep);
	DEBUG(( EFI_D_ERROR, "Begin 4GB~6GB Cross Page Address Test\r\n"));
    StartAddr = 0x100000000;
	//TestLen = 0x80000000;
	JumpStep = 0x20000;
	//JumpAddrTest(StartAddr, TestLen, JumpStep);
    ...
    DEBUG(( EFI_D_ERROR, "Begin 4GB~6GB Cross 0x100 Address Test\r\n"));
    StartAddr = 0x100000000;
	//TestLen = 0x80000000;
	JumpStep = 0x100;
	//JumpAddrTest(StartAddr, TestLen, JumpStep);
    ...
    DEBUG(( EFI_D_ERROR, "Begin 4GB~6GB Cross BankGroup Address Test\r\n"));
    StartAddr = 0x100000000;
	//TestLen = 0x80000000;
	JumpStep = 0x40;
	...
	DEBUG(( EFI_D_ERROR, "Begin 4GB~6GB Cross 0x08 Address Test\r\n"));
    StartAddr = 0x100000000;
	//TestLen = 0x80000000;
	JumpStep = 0x8;
   ...
   //TestLen = 0x70000000;
    //RandMemTest(StartAddr, TestLen);
   


	

    DEBUG(( EFI_D_ERROR, "Begin 4GB~6GB Write to Read Test\r\n"));
	StartAddr = 0x100000000;
	//TestLen = 0x80000000;
	JumpStep = 0x8000;
	//Write2ReadTest(StartAddr, TestLen, JumpStep);



	
    DEBUG(( EFI_D_ERROR, "Begin 4GB~6GB Address Test\r\n"));

    StartAddr = 0x100000000;
	//TestLen = 0x80000000;
	//DXEAddrtest(StartAddr, TestLen); 
    ...
    StartAddr = 0x100000000;
	TestLen = 0x20000000;
	DEBUG(( EFI_D_ERROR, "Begin Master Socket 4GB~4.5GB Address Test\r\n"));
	DXEAddrtest(StartAddr, TestLen); 
	
    StartAddr = 0x100000000;
	TestLen = 0x80000000;
    DEBUG(( EFI_D_ERROR, "Begin Master Socket 4GB~6GB Normal Test\r\n"));   
	DXEDramTest(StartAddr, TestLen, Pattern);

	StartAddr = 0x380000000;
	TestLen = 0x20000000;
	DEBUG(( EFI_D_ERROR, "Begin Master Socket 14GB~14.5GB Address Test\r\n"));
	DXEAddrtest(StartAddr, TestLen); 
	
    StartAddr = 0x380000000;
	TestLen = 0x80000000;
    DEBUG(( EFI_D_ERROR, "Begin Master Socket 14GB~16GB Normal Test\r\n"));   
	DXEDramTest(StartAddr, TestLen, Pattern);


	StartAddr = 0x400000000;
	TestLen = 0x20000000;
	DEBUG(( EFI_D_ERROR, "Begin Master Socket 16GB~16.5GB Address Test\r\n"));
	DXEAddrtest(StartAddr, TestLen); 
	
    StartAddr = 0x400000000;
	TestLen = 0x80000000;
    DEBUG(( EFI_D_ERROR, "Begin Master Socket 16GB~18GB Normal Test\r\n"));   
	DXEDramTest(StartAddr, TestLen, Pattern);	 
	#endif

}
//LGE-20170815 DXE MEMTEST CODE -E
...
#if 0
   //Begin Memory Test		
   DEBUG(( EFI_D_ERROR, "Begin Burn in Memory Test\r\n")); 
   DXEBurnIn();   		
   DEBUG((EFI_D_ERROR, "Memory Test Finish \r\n"));	 
#endif
