  ...
  )
{
  ...
  REPORT_STATUS_CODE (
    EFI_PROGRESS_CODE, 
    (EFI_SOFTWARE_EFI_RUNTIME_SERVICE | BYO_DXE_RT_PC_VARIABLE_CLEAN_UP)
    );
  ...
  )
{
  ...
  if (GuidHob != NULL) {
    ...
    if (FtwLastWriteData->TargetAddress == TempVariableStoreHeader) {
      DEBUG ((EFI_D_INFO, "Variable: NV storage is backed up in spare block: 0x%x\n", (UINTN) FtwLastWriteData->SpareAddress));
      TempVariableStoreHeader = FtwLastWriteData->SpareAddress;
    }	
  }
  ...
  mVariableModuleGlobal->VariableGlobal.NonVolatileVariableBase = VariableStoreBase;
  VariableStoreHeader = (VARIABLE_STORE_HEADER *)(UINTN)VariableStoreBase;
  NvStoreStatus = GetVariableStoreStatus (VariableStoreHeader);
  if (NvStoreStatus == EfiValid) {
    ...
    DEBUG((EFI_D_INFO, "Variable Store header is corrupted, and we use the build settings\n"));
    NextVariable  = GetStartPointer ((VARIABLE_STORE_HEADER *)(UINTN)VariableStoreBase);
    mVariableModuleGlobal->NonVolatileLastVariableOffset = (UINTN) NextVariable - (UINTN) VariableStoreBase;
  }
  ...
  if (NvStoreStatus == EfiValid) {
    ...
    VariableStoreHeader = (VARIABLE_STORE_HEADER *)(UINTN)mNvVariableCache;
    SetMem (mNvVariableCache, (UINTN)VariableStoreLength, 0xff);
    CopyGuid (&VariableStoreHeader->Signature, &gEfiVariableGuid);
    VariableStoreHeader->Size        = (UINT32)VariableStoreLength;
