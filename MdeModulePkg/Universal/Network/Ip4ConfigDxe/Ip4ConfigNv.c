  ...
  )
{
  ...
  UINTN                         BufferSize;
  EFI_GUID IfrNvGuid =  EFI_NIC_IP4_CONFIG_NVDATA_GUID;
  IP4_CONFIG_IFR_NVDATA    Ip4IfrNvData;

  Instance = IP4_CONFIG_INSTANCE_FROM_SAVE_NOTIFY(This);
  BufferSize = sizeof (IP4_CONFIG_IFR_NVDATA);
  if (HiiGetBrowserData (&IfrNvGuid, L"EfiNicIp4ConfigVariable", BufferSize, (UINT8 *) (&Ip4IfrNvData))) {
    Status = Ip4ConfigConvertIfrNvDataToDeviceConfigData (&Ip4IfrNvData, Instance);
  }
  ...
  )
{
  ...
  //
  // Setup hotkey save function synchronizing with Byo setup browser.
  // 
  SetupSaveNotify = &Instance->SetupSaveNotify;
  SetupSaveNotify->SaveValue = Ip4SaveValue;
