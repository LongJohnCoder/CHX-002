  ...
  )
{
  ...
  NvramBuffer = AllocatePool(Length);
  if (NvramBuffer == NULL) {
    Status = EFI_OUT_OF_RESOURCES;
    goto ProcExit;	
  }
  ...
  if(CompareMem(Buffer, NvramBuffer, Length) == 0){
    DEBUG((EFI_D_INFO, "Spare is equal to Target!\n"));		
    Status = EFI_SUCCESS;
    goto ProcExit;		
  }		
...
ProcExit:
  ...
  if(NvramBuffer){FreePool(NvramBuffer);}	
  return Status;
}
