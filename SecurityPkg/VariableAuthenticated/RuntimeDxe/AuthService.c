  ...
  )
{
  ...
  if (Variable.CurrPtr != NULL) {         // "SecureBootEnable" present
    if(mPlatformMode == SETUP_MODE){      // "PK" absent, but "SecureBootEnable" present, so remove it. 
      Status = UpdateVariable (
  ...
  } else if (mPlatformMode == USER_MODE) {                  // "SecureBootEnable" absent and in user mode.
    //
