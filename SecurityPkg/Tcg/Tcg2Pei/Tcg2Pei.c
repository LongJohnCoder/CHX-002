...
//    ASSERT_EFI_ERROR (Status);
    PcdSet32(PcdTpm2HashMask, NewTpm2PcrMask);
  }
}
...
STATIC CHAR16 gDefaultSCrtmVersion[] = {L"{D20BC7C6-A1A5-415c-AE85-38290AB6BE04}"};
  ...
  )
{
  ...
  CrtmStr = (CHAR16*)PcdGetPtr(PcdFirmwareVersionString);
  if(CrtmStr[0] == 0){
    CrtmStr = gDefaultSCrtmVersion;
  }  

  TcgEventHdr.PCRIndex  = 0;
  TcgEventHdr.EventType = EV_S_CRTM_VERSION;
  TcgEventHdr.EventSize = (UINT32)StrSize(CrtmStr);
  ...
  )
{
  ...
  if (PcdGet8(PcdTpmInstanceId) != TPM_DEVICE_2_0_DTPM){
