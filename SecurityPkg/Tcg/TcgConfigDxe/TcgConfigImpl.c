...
EFI_GUID  gSetupEnterGuid  = TSE_SETUP_ENTER_GUID;
  ...
  )
{
  ...
  *Progress = Request;
  ...
  if(PrivateData->CurrentChoice!=PHYSICAL_PRESENCE_NO_ACTION){    // not default.
    Configuration.TpmOperation = PrivateData->CurrentChoice;
  }else{
  ...
  )
{
  ...
  if ((Action != EFI_BROWSER_ACTION_CHANGED && Action != EFI_BROWSER_ACTION_CHANGING) || QuestionId != KEY_TPM_ACTION) {
    return EFI_UNSUPPORTED;
  }

  SavePpRequest (Value->u8);
  PrivateData->CurrentChoice = Value->u8;
...
// check protocol really installed or not?
...
//------------------------------------------------------------------------------
  PrivateData = (TCG_CONFIG_PRIVATE_DATA*)Context;
...
// enable, enable+activate, enable+activate+clear, enable+activate+clear+enable+activate is always valid.
// disable, activate, deactivate, deactivate+disable is valid when enabled.
// clear, clear+enable+activate is valid when enabled and activated
  ...
  HiiCreateOneOfOptionOpCode (
    OptionsOpCodeHandle,
    STRING_TOKEN (STR_TPM_ENABLE_ACTIVATE),
    0,
    EFI_IFR_NUMERIC_SIZE_1,
  ...
  HiiCreateOneOfOptionOpCode (
    OptionsOpCodeHandle,
    STRING_TOKEN (STR_TPM_ENABLE_ACTIVATE_CLEAR),
    0,
    EFI_IFR_NUMERIC_SIZE_1,
  ...
  HiiCreateOneOfOptionOpCode (
    OptionsOpCodeHandle,
    STRING_TOKEN (STR_TPM_ENABLE_ACTIVATE_CLEAR_E_A),
    0,
    EFI_IFR_NUMERIC_SIZE_1,
  ...
  if (PrivateData->TcgProtocol != NULL) {
    ...
    if (!EFI_ERROR (Status)) {
      ...
      if(TpmEnable){
        ...
        HiiCreateOneOfOptionOpCode (
          OptionsOpCodeHandle,
          STRING_TOKEN (STR_TPM_DEACTIVATE),
          0,
          EFI_IFR_NUMERIC_SIZE_1,
        ...
        HiiCreateOneOfOptionOpCode (
          OptionsOpCodeHandle,
          STRING_TOKEN (STR_TPM_DEACTIVATE_DISABLE),
          0,
          EFI_IFR_NUMERIC_SIZE_1,
      ...
      if(TpmEnable && TpmActivate){
        HiiCreateOneOfOptionOpCode (
          OptionsOpCodeHandle,
          STRING_TOKEN (STR_TPM_CLEAR),
          0,
          EFI_IFR_NUMERIC_SIZE_1,
        ...
        HiiCreateOneOfOptionOpCode (
          OptionsOpCodeHandle,
          STRING_TOKEN (STR_TPM_CLEAR_ENABLE_ACTIVATE),
          0,
          EFI_IFR_NUMERIC_SIZE_1,
  ...
  HiiCreateOneOfOpCode (
    StartOpCodeHandle,                         
    KEY_TPM_ACTION,                            
    TCG_CONFIGURATION_VARSTORE_ID,                 
    (UINT16)OFFSET_OF(TCG_CONFIGURATION, TpmOperation),
    STRING_TOKEN (STR_TPM_OPERATION),
    STRING_TOKEN (STR_TPM_OPERATION_HELP), 
  ...
  HiiUpdateForm (PrivateData->HiiHandle, NULL, TCG_CONFIGURATION_FORM_ID, StartOpCodeHandle, EndOpCodeHandle);
