...
{
  CHAR16                          *StringPtr_temp;  
  CHAR16                          *Str_temp;    
  UINTN                             StrCount;
  UINTN                             StrCount_temp;  
  if (NULL == StringPtr) return EFI_INVALID_PARAMETER;
  StringPtr_temp = AllocateZeroPool(StrLen(StringPtr)*sizeof(CHAR16)+2);
  StrCount = 0;
  StrCount_temp = 0;
  Str_temp = StringPtr;
  while (*Str_temp == L' ')
    Str_temp++;
  while (*Str_temp != CHAR_NULL) {
     if ((CHAR16)*Str_temp == L' ') {
       StrCount_temp += 1; 
       if (StrCount_temp > 1) {
         Str_temp ++;
         continue;
     ...
     StringPtr_temp[StrCount] = *Str_temp;     
     Str_temp ++; 
     StrCount++;
  }
  if (StringPtr_temp[StrCount-1] == L' ' ) {
    StringPtr_temp[StrCount-1] = CHAR_NULL;
  } else {
    StringPtr_temp[StrCount] = CHAR_NULL;
  }
  StrCpy(StringPtr,StringPtr_temp);
  FreePool(StringPtr_temp);
  return EFI_SUCCESS;
}
/**
  ...
  case EFI_IFR_NUMERIC_OP:
    if (Selected) {
      ...
      *OptionString = AllocateZeroPool (BufferSize);
      ...
      CopyMem (OptionString[0], FormattedNumber, Number);
      ...
      if ((Question->QuestionFlags & EFI_IFR_FLAG_CALLBACK)  != 0) {
        ...
        *StringPtr = 0;
        ...
        if (EFI_ERROR (Status)) {
          ...
          if (EFI_ERROR (Status)) {
            PasswordCallback (Selection, MenuOption, NULL);   // reset state mathine.
            FreePool (StringPtr);
            return Status;
          }
          ...
          if (EFI_ERROR (Status)) {
            ...
            FreePool (StringPtr);
//-         return Status;
//- Even if old password check failed, we maybe need to do reflash as some items updated.
            DEBUG((EFI_D_ERROR, "Old Password is incorrect, but we return SUCCESS to let broswer refresh!\n"));
            return EFI_SUCCESS;
          }
        }
      } else {
      ...
      if (StrCmp (StringPtr, TempString) == 0) {
        ...
        if (EFI_ERROR (Status)) {
          ...
          if ((Question->QuestionFlags & EFI_IFR_FLAG_CALLBACK) != 0) {
            ...
            if(EFI_ERROR(Status)){
              do {
                CreateDialog (4, TRUE, 0, NULL, &Key, gEmptyString, gResultFailed, gPressEnter, gEmptyString);
              } while (Key.UnicodeChar != CHAR_CARRIAGE_RETURN);
            }else{
              do {
                CreateDialog (4, TRUE, 0, NULL, &Key, gEmptyString, gResultSuccess, gPressEnter, gEmptyString);
              } while (Key.UnicodeChar != CHAR_CARRIAGE_RETURN);
            }
          } else {
            CopyMem (Question->BufferValue, StringPtr, Maximum * sizeof (CHAR16));
  ...
  )
{
  ...
  //
  LineWidth      = (UINT16) (gHelpBlockWidth - 2);
