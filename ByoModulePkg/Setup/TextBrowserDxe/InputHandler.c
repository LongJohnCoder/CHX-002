  ...
  )
{
  ...
  do {
    ...
    case CHAR_CARRIAGE_RETURN:
      if (GetStringWidth (StringPtr) >= ((Minimum + 1) * sizeof (CHAR16)) || *StringPtr == CHAR_NULL) {
  ...
  )
{
  ...
  //
  if (gDirection == SCAN_RIGHT || gDirection == SCAN_LEFT) {
    ManualInput = FALSE;
  } else {
    ManualInput = TRUE;
  }
  ...
  do {
    ...
    if (0xE000 == gDirection) {
      Key.UnicodeChar = gDirectInputNum;
      Key.ScanCode  = 0;
      Count = 0;
      gDirection = 0;
      gDirectInputNum = CHAR_NULL;
      goto ShowInputNumber;
    }
    if (gDirection == SCAN_RIGHT || gDirection == SCAN_LEFT) {
      ...
      case SCAN_RIGHT:
        ...
        if ((Step != 0) && !ManualInput) {
          ...
          if (Question->Operand == EFI_IFR_NUMERIC_OP) {
            gST->ConOut->SetAttribute (gST->ConOut, EFI_WHITE | EFI_BACKGROUND_BLUE); 
            Column = MenuOption->OptCol;
            PrintStringAt (Column, Row, NUMERIC_BACKGROUND_STRING);          
          }
    ...
    default:
      ...
      if (ManualInput) {
        ...
        if (Question->Operand == EFI_IFR_DATE_OP ||
          ...
          gST->ConOut->SetAttribute (gST->ConOut, PcdGet8 (PcdBrowserFieldTextHighlightColor) | FIELD_BACKGROUND);
        }
  ...
  )
{
  ...
  UINTN                   CurrentHighlightOptionIndex;
  UINTN                   Start;
  UINTN                   End;
  UINTN                   Top;
  UINTN                   Bottom;
  ...
  Start  = (DimensionsWidth - PopUpWidth - POPUP_FRAME_WIDTH) / 2 + gScreenDimensions.LeftColumn + 3;
  End    = Start + PopUpWidth + 2;
  Top    = gScreenDimensions.TopRow + 5;
  Bottom = gScreenDimensions.BottomRow - 6;
  ...
  do {
    ...
    //
    if (!bDrawed) ClearLines (Start, End, Top, Bottom, POPUP_TEXT | POPUP_BACKGROUND);
    ...
    if (!bDrawed) PrintCharAt (Start, Top, Character);
    for (Index = Start; Index + 2 < End; Index++) {
      Character = BOXDRAW_HORIZONTAL;
      if (!bDrawed)PrintChar (Character);
    }
    ...
    for (Index = Top + 1; Index < Bottom; Index++) {
      if (!bDrawed)PrintCharAt (Start, Index, Character);
      if (!bDrawed)PrintCharAt (End - 1, Index, Character);
    }
   ...
   if (HighlightOptionIndex < CurrentHighlightOptionIndex) {
     //
     //Up
     //
     if (HighlightOptionIndex > TopOptionIndex) {
       DrawedAllOption = FALSE;
     } else {
       DrawedAllOption = TRUE;     
     }
   } else {
     //
     //Down
     //   
     if (HighlightOptionIndex < (TopOptionIndex + MenuLinesInView - 1)) {
       DrawedAllOption = FALSE;
     } else {
       DrawedAllOption = TRUE;     
     }
    ...
    for (Index = TopOptionIndex; (Index < PopUpMenuLines) && (Index2 < Bottom); Index++) {
      ...
      if (Index == HighlightOptionIndex) {
          ...
          ClearLines (Start + 2, End - 2, Index2, Index2, POPUP_TEXT | POPUP_BACKGROUND);
          ...
          if (DrawedAllOption) {
            ...
            if ( Index == CurrentHighlightOptionIndex || !bDrawed) {
              ClearLines (Start + 2, End - 2, Index2, Index2, POPUP_TEXT | POPUP_BACKGROUND);                  
              gST->ConOut->SetAttribute (gST->ConOut, POPUP_TEXT | POPUP_BACKGROUND);
              PrintStringAt (Start + 2, Index2, StringPtr);
            }          
    ...
    if (!bDrawed)PrintCharAt (Start, Bottom, Character);
    for (Index = Start; Index + 2 < End; Index++) {
      Character = BOXDRAW_HORIZONTAL;
      if (!bDrawed)PrintChar (Character);
    }
    ...
    if (ShowUpArrow) {
      ...
      PrintCharAt ( ((Start + End) / 2), Top, GEOMETRICSHAPE_UP_TRIANGLE);    
     } else {
      gST->ConOut->SetAttribute (gST->ConOut, EFI_GREEN | POPUP_BACKGROUND);
      PrintCharAt ( ((Start + End) / 2), Top, BOXDRAW_HORIZONTAL);
    }
	 
    if (ShowDownArrow) {
      gST->ConOut->SetAttribute (gST->ConOut, EFI_RED| EFI_BACKGROUND_BLUE);
      PrintCharAt ( ((Start + End) / 2), Bottom, GEOMETRICSHAPE_DOWN_TRIANGLE);    
     } else {
      gST->ConOut->SetAttribute (gST->ConOut, EFI_GREEN | POPUP_BACKGROUND);
      PrintCharAt ( ((Start + End) / 2), Bottom, BOXDRAW_HORIZONTAL);
    }
    bDrawed = TRUE;
    if (Question->MaxContainers > (Bottom - Top - 1)) {
      DrawedAllOption = TRUE;
    } else {
      DrawedAllOption = FALSE;
    }
