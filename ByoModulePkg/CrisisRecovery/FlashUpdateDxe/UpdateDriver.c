...
/**
  a notify function for flash update procedur
  ...
  )
{
  ...
  CHAR16                        *CapsuleExtVarName   = EFI_CAPSULE_EXT_VARIABLE_NAME;
  EFI_GUID                      EfiCapsuleVendorGuid = EFI_CAPSULE_VENDOR_GUID;
  BYO_CAPSULE_EXTEND            CapExt = {0, 0};
  UINTN                         Size;
  BOOLEAN                       bCapExt;
  UINT32                        CRC32;
  UINTN                         Delay;
  ...
  if (BootMode == BOOT_ON_FLASH_UPDATE) {
    ...
    //
    // Get rom image CRC32 from variable
    //
    Size   = sizeof(CapExt);
    Status = gRT->GetVariable (
                    CapsuleExtVarName,
                    &EfiCapsuleVendorGuid,
                    NULL,
                    &Size,
                    (VOID *) &CapExt
                    );
  ...
  //
  // Locate the top-swap protocol
  //
  Status = gBS->LocateProtocol (
                  &gFirmwareReadTopSwapProtocolGuid,
                  NULL,
                  &mTopSwapper
                  );
  if (EFI_ERROR(Status)) {
    DEBUG ((EFI_D_ERROR,"Locate TopSwap protocol Status = %r\n", Status));
  }
  do {
    FlashUpdate (BootMode, UpdateBehavior, FDImageBaseAddress, FDImageLength);
    Status = ReadFlashAndVerify (UpdateBehavior, FDImageBaseAddress);
    if (EFI_ERROR (Status)) {
  ...
  //
  if (mTopSwapper != NULL) {
    Status = mTopSwapper->SetSwap (mTopSwapper, FALSE);
    if (EFI_ERROR (Status)) {
      return Status;
    }
  }
