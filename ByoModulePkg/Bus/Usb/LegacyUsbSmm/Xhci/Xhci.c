...
/*
    ...
    while (Next != &mEhcList) {
        ehci = CR(Next, USB2_HC_DEV, Link, USB2_HC_DEV_SIGNATURE);
        Status= ehci->Usb2Hc.AsyncInterruptOnlyTransfer(
                    &(ehci->Usb2Hc),
                    IsNewTransfer,
                    PollingInterval,
                    CallBackFunction,
...
)
{
  ...
  XhcSetOpRegBit (Xhc, XHC_USBSTS_OFFSET, XHC_USBSTS_EINT);    // Set BIT3 to clear it
