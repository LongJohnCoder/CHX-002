  ...
  )
{
  ...
  //
    ...
    for (Count = 0; Count < 8; Count ++) {
      if (UsbMass->VendorID[Count] & 0x80) {
        FlagNotASCII = TRUE;
      }
    }

    for (Count = 0; Count < 16; Count ++) {
     if(UsbMass->ProductID[Count] & 0x80) {
      FlagNotASCII = TRUE;
     }
    }
...
//determine if ProductID is full space 
  ...
  if(FlagVendorSpace&&FlagProductSpace){
    ...
    if(FlagNoRetry && FlagNotASCII) {
      ...
      UsbMass->VendorID[0] = 'U';
      UsbMass->VendorID[1] = 'S';
      UsbMass->VendorID[2] = 'B';
      UsbMass->VendorID[3] = ' ';
      UsbMass->VendorID[4] = ' ';
      UsbMass->VendorID[5] = ' ';
      UsbMass->VendorID[6] = ' ';
      UsbMass->VendorID[7] = ' ';
      UsbMass->ProductID[0] = 'F';
      UsbMass->ProductID[1] = 'l';
      UsbMass->ProductID[2] = 'a';
      UsbMass->ProductID[3] = 's';
      UsbMass->ProductID[4] = 'h';
      UsbMass->ProductID[5] = ' ';
      UsbMass->ProductID[6] = 'D';
      UsbMass->ProductID[7] = 'i';	  
      UsbMass->ProductID[8] = 's';
      UsbMass->ProductID[9] = 'k';
      UsbMass->ProductID[10] = ' ';
      UsbMass->ProductID[11] = 0x30+USBDeviceNum;
      UsbMass->ProductID[12] = ' ';
      UsbMass->ProductID[13] = ' ';
      UsbMass->ProductID[14] = ' ';
      UsbMass->ProductID[15] = ' ';
    }
