...
**/
UINT8
LegacyWriteStandardCmos_original (
  IN UINT8  Index,
  IN UINT8  Value
  )
{
...

{
	...
	if((Index>=0xA)&&(Index<=0xD)){
	  RegisterB.Data		= LegacyReadStandardCmos (0x0B);
	  if(RegisterB.Bits.Set==0){
	  RegisterA.Data	= LegacyReadStandardCmos (0x0A);
	...
	return LegacyWriteStandardCmos_original(Index,Value);
