  ...
  )
{
  ...
  for (VbeModeNumber = ReadUnaligned16 (ModeNumberPtr);
    ...
    if (BiosVideoPrivate->VbeModeInformationBlock->XResolution == 1024 &&
      ...
      //
      // Record the minimum resolution required by Windows 8
      //
      MinimumMode = ModeNumber;
      ModeFound = TRUE;
    }
    ...
    if (BiosVideoPrivate->VbeModeInformationBlock->XResolution == PcdGet32(PcdVideoHorizontalResolution) &&
        BiosVideoPrivate->VbeModeInformationBlock->YResolution == PcdGet32(PcdVideoVerticalResolution)
        ) {
      ModeFound = TRUE;
      PreferMode = ModeNumber;
    }
