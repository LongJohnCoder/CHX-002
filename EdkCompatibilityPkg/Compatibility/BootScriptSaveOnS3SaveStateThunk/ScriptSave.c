...
{
  EFI_PHYSICAL_ADDRESS           EntryPointAddr;
  EFI_PHYSICAL_ADDRESS           ContextAddr;
  
  EntryPointAddr = VA_ARG (Marker, EFI_PHYSICAL_ADDRESS);

  //
  // Register callback
  //
  ContextAddr    = EntryPointAddr;
  EntryPointAddr = (EFI_PHYSICAL_ADDRESS)(UINTN)FrameworkBootScriptDispatchStub;
  ...
  )
{
  ...
  //          = Duration * 1000ns * LoopTimes / 1S00ns
  ...
  )
{
  ...
  EntryPointAddr = VA_ARG (Marker, EFI_PHYSICAL_ADDRESS);
  ContextAddr    = VA_ARG (Marker, EFI_PHYSICAL_ADDRESS);
